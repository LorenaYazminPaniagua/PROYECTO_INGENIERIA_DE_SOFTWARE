...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['idPedido'], $_POST['accion'])) {
    ...
    // Establecer variable de sesión para el trigger o procedimiento
    $conexion->query("SET @id_empleado_sesion := $idEmpleado");

    switch ($accion) {
        case 'enviado':
            $stmt = $conexion->prepare("CALL CambiarPedidoAEnviado(?)");
            break;
        case 'cancelado':
    ...
    header("Location: PedidosAceptados.php");
    exit;
}
...
$query = "SELECT * FROM VistaPedidosAceptados ORDER BY idPedido, idDetallePedido";
                    ...
                    <div class="card">
                        ...
                        <div class="acciones">
                            <form method="POST" style="display:inline-block; margin-right: 10px;">
                                ...
                                <button type="submit" class="btn-recibido">Enviar</button>
                            </form>
