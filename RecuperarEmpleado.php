...
if (isset($_GET['idEmpleado']) && is_numeric($_GET['idEmpleado'])) {
    $idProducto = intval($_GET['idEmpleado']);

    // Llamar al procedimiento almacenado EliminarProducto
    $sql = "CALL RecuperarEmpleado(?)";
    ...
    if ($stmt->execute()) {
        ...
        header("Location: EmpleadosInactivos.php");
        exit();
    } else {
        $error = $stmt->error;
        $stmt->close();
    ...
    $conexion->close();
    header("Location: EmpleadosInactivos.php"); // Redirigir si no se recibe ID válido
    exit();
}
?>